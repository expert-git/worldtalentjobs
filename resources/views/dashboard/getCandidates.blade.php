...
<section>
    ...
    <div style="margin-top:5%" class="col-md-10" >
		<div class="header_listtop">
			<ul>
				<li><a href="/inspector/getCandidates/{{$emp}}/{{$id}}/0">All ({{$all}})</a></li>
				<li><a href="/inspector/getCandidates/{{$emp}}/{{$id}}/1">Shortlisted ({{$shortlisted}})</a></li>
				<li><a href="/inspector/getCandidates/{{$emp}}/{{$id}}/2">Rejected ({{$rejected}})</a></li>
				<li><a href="/inspector/getCandidates/{{$emp}}/{{$id}}/3">Scheduled for interview ({{$scheduled}})</a></li>
				<li><a href="/inspector/getCandidates/{{$emp}}/{{$id}}/4">Spams ({{$spam}})</a></li>
			</ul>
		</div>   
  ...
  @endforeach
				...
				<a href="/inspector/createConversation/{{$j->employer_id}}/{{$j->jobseeker_id}}">	<span class="message"> <i class="far fa-envelope"></i> Message </span></a>
				<a href="/inspector/markspam/{{$j->aj}}/4">	<span class="markspan">Mark as Spam</span>	</a>				
				</div>
			</div>
			...
			<div class="row canditate_details">
				<div class="col-md-4">
				<a href="/inspector/postdetails/{{$j->job_id}}/{{$j->jobseeker_id}}">	<h3>{{isset($j->full_name )?$j->full_name :NULL}}</h3>
            ...
            swal({
                 ...
                 content: {
                     ...
                     if (time) {
                        ...
                        window.location="/inspector/markspam/"+jid+"/3?jobid="+jobid+"&time="+time;
                          }
                          });
