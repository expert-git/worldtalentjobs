...
<section>
    <div style="padding:0px 50px" class="container">
		<!--<div class="row">
		...
		<div class="row" style="padding-top:30px; padding-bottom:60px;">
			<div class="col-md-12">
				<div style="float: right">
					...
					<button onclick="PrintElem('#employer_datatable')" class="btnde btnprint"><img width="30px" height="30px" src="{{url('/img/print.png')}}" alt="">&nbsp Print</button>
				...
				<table id="employer_datatable" class="table table-striped instable" style="width:100%">
					<thead>
						 <tr class="firstrow">
							<th>Employer.ID</th>
							<th>Company Name</th>
					...
					<tbody style="display: table-row-group;">
						@forelse($data as $j)
						<tr>
							<td><a href="/inspector/employerdetail/{{$j->employer_id}}">{{$j->employer_id}}</a></td>
							<td><a href="/inspector/employerdetail/{{$j->employer_id}}">{{$j->companyname}}</td>
							<td><a href="/inspector/employerdetail/{{$j->employer_id}}">{{$j->ind}}</td>
							<td><a href="/inspector/employerdetail/{{$j->employer_id}}">{{$j->em}}</td>
							<td><a href="/inspector/employerdetail/{{$j->employer_id}}">{{$j->ContactPhone}}</td>
							<td><a href="/inspector/employerdetail/{{$j->employer_id}}">{{$j->created_at}}</td>
							<td><a href="/inspector/employerdetail/{{$j->employer_id}}">{{$j->cnt}}</td>
						</tr>
						@empty
						<tr>
	...
	<script>
		$(document).ready(function(){
			...
			$('.dpk1').change(function() {
				...
				window.location.replace("{{url('/inspector/employerlist')}}"+"/"+sdate+"/"+ndate);
		...
		function Popup(data)
		{
			...
			mywindow.document.write('<style> table{border-collapse: collapse; font-size: 14px;} tr { border: 1px solid rgb(128, 128, 128); padding: 40px; line-height: 25px;' +
