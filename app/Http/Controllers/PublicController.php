...
class PublicController extends Controller
{
    public function index()
    {
        ...
        return view('public.'.$theme,compact('portal_option','divisions','jobs','category','division','jobseekerlist','employerList','catagorys','catagorysWiseJobcount','get_cats','hotjobs','recemtJobs','populjobs','homenotice'));
    }
...
// retrive homepage notice
    ...
    public function createConversation($js)
    {
        ...
        return redirect("/employer/message");
...
public function indexpage()
{
    ...
    return view('public.index',compact('divisions'));
}
...
{
    $conv=conversation::where('employer',Auth::guard('employer')->user()->id)
    ->get();
}
public function jsInbox()
{
    ...
    $conv=conversation::where('jobseeker',Auth::guard('jobseeker')->user()->id)->get();
    $total=Applied_job::where("jobseeker_id","=",Auth::guard('jobseeker')->user()->id)->count();

    return view('jobseeker.messages',compact('total','applied','jobseekerdetails','conv'));
}
...
$rec=$data->all();
  ...
  //  $frm->name=$data->name;
  //$path = $data->file('file')->store('file');
  //$destinationPath = 'uploads';
  //$d=$file->move($destinationPath,$file->getClientOriginalName());
//  $path = \Storage::putFile('public', $data->file('file'));
...
//echo $d;
  ...
  Mail::raw('Text to e-mail'. json_encode($rec, JSON_UNESCAPED_SLASHES), function ($m){
    $m->from('primetechsolution1234@gmail.com', 'WTJ');

    $m->to("omkar2008@gmail.com","WTJ")->subject('Someone contacted you!');
});
    print_r($data->name);
...
private function selecttheme(){
    ...
    public function apply($id)
    {
        ...
        return view('jobseeker.apply',compact('jobs'));
...
// show jobdetails by id
    ...
    //return $catid;
   $relatedJobs=DB::table('jobs')->where('jobs.catagory_id',$catid)->where('jobs.id','!=',$id)->join('empprofiles', 'jobs.employer_id', '=', 'empprofiles.employer_id') ->join('divisions', 'jobs.division_id', '=', 'divisions.id')->select('jobs.*','divisions.name as divname','empprofiles.companyname as companyname','empprofiles.companylogo as clogo')->
take(4)->orderByRaw("Rand()")->get();
